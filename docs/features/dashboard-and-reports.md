...
!!! warning "WeasyPrint needs system libraries"
    PDF rendering goes through [WeasyPrint](https://doc.courtbouillon.org/weasyprint/stable/),
    which depends on Pango and Cairo at the OS level — `pip install` alone
    does not satisfy that on most systems. Follow WeasyPrint's own
    [installation docs](https://doc.courtbouillon.org/weasyprint/stable/first_steps.html#installation)
    for your platform before relying on PDF generation. The dashboard's HTML
    *preview* (no PDF) works without these system libraries, since the
    renderer imports WeasyPrint lazily — only the download/email paths need
    it.
...
Emailing a report is covered in [Emailing reports](../guides/email.md).
